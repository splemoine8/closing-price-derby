...
// Backfill script to capture ALL sales during the competition period
// This bypasses the 7-day limitation by fetching a larger window
...
// Calculate days for soldWithin parameter (add buffer)
const totalDays = Math.ceil((COMPETITION_END - COMPETITION_START) / (1000 * 60 * 60 * 24)) + 7; // +7 day buffer

console.log(`🏁 Competition Backfill Script`);
console.log(`📅 Period: ${COMPETITION_START.toISOString()} to ${COMPETITION_END.toISOString()}`);
console.log(`📊 Using soldWithin=${totalDays} to capture entire period\n`);
...
async function fetchFromRedfin(regionId, cityName) {
  console.log(`  🏠 Fetching ALL properties for ${cityName} (region ${regionId})...`);
  ...
  while (hasMore) {
    const url = `${BASE_URL}/properties/search-sold?regionId=${regionId}&soldWithin=${totalDays}&limit=${limit}&offset=${offset}`;
    ...
    for (let i = 0; i < allSales.length; i += batchSize) {
      ...
      try {
        ...
        if (error) {
          ...
          added += batch.length; // Simplified counting
        }
      } catch (err) {
