...
interface ZipDetailModalNewProps {
  ...
  baseline: number;
  highestSale: SaleData | null;
  highestSaleMultiplier: string;
  scorePct?: number | null;  // NEW: Passed from parent
}
...
const ZipDetailModalNew = ({ 
  ...
  // SIMPLIFIED: Just use the multiplier passed from parent
  const saleMultiple = highestSaleMultiplier || '—';

  // SIMPLIFIED Score Breakdown - no calculations, just display
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t pt-2">
              ...
              <span className={`font-medium ${isAboveBaseline ? 'text-green-600' : 'text-gray-500'}`}>
                {isAboveBaseline && scorePct !== null && scorePct !== undefined ? `+${scorePct.toFixed(1)}%` : '—'}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            <>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {topSales
                    .filter(sale => !displaySale || sale.address !== displaySale.address || sale.date !== displaySale.date)
                    .slice(0, 5)
                    .map((sale, index) => (
